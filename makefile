...
fh6_telemetry_SRCS := $(SRCDIR)/main.cpp $(SRCDIR)/socket_setup.cpp $(SRCDIR)/engine_rpm.cpp $(SRCDIR)/gforce.cpp $(SRCDIR)/map.cpp $(SRCDIR)/date.cpp $(SRCDIR)/car_info.cpp 
...
windows: $(BUILDDIR) $(BUILDDIR)/fh6_telemetry.exe $(BUILDDIR)/capture_data.exe 
...
$(BUILDDIR)/%_win.o: $(SRCDIR)/%.cpp
	$(WIN_CXX) -Wall -Wextra -Werror -O3 -std=c++20 \
		-I$(WIN_SDL)/include \
		-I$(WIN_TTF)/include \
		-c $< -o $@
