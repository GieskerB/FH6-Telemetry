...
bool handle_telemetry_arg(std::string arg, std::vector<telemetry_variant_t>& telemetries) {
    ...
    for (unsigned char i = 0; i < TELEMETRY_COUNT; ++i) {
        if (telemetry_name == TELEMETRIES[i]) {
            switch (i) {
                case 0:
                    telemetries.emplace_back(car_info_t{});
                    break;
                case 1:
                    telemetries.emplace_back(engine_rpm_t{});
                    break;
                case 2:
                    telemetries.emplace_back(gforce_t{});
                    break;
                case 3:
                    telemetries.emplace_back(map_t{});
                    break;
                case 4:
                    telemetries.emplace_back(race_info_t{});
                    break;
                case 5:
                    telemetries.emplace_back(wheel_info_t{});
                    break;
            }
            // Cannot instanace same telemetry more then once
            if (specify_size) {
                ...
                try {
                    ...
                    std::visit([&](auto& telem) { telem.init(size); }, telemetries.back());

                } catch (std::invalid_argument& e) {
                    std::cerr << "Cannot parse " << tmp << " into a number!\n";
                    std::cerr << e.what() << '\n';
                    return true;
                ...
                try {
                    std::visit([&](auto& telem) { telem.init(); }, telemetries.back());
                } catch (std::runtime_error& e) {
                    std::cerr << e.what() << '\n';
                    return true;
                }
