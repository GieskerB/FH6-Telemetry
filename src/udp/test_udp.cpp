    ...
    if (!file.is_open()) {
        ...
        if (!file) {
            ...
            if (!file) {
                ...
                std::cerr << "Could not open file '" << make_filename(folder_number, current_file)
                          << "': " << strerror(errno) << "\n";
                exit(EXIT_FAILURE);
            }
        }
...
bool run_recorded(const char* test_mode_str) { return strncmp("recorded", test_mode_str, 9) == 0; }
