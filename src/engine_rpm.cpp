...
void update(const fh6_data & data_out) {
    ...
    // ==========================================
    // 2. DATA CALCULATIONS (Kept lean, avoiding std::to_string heap allocations)
    // ==========================================
    int speed_kmh = std::clamp(static_cast<int>(std::abs(data_out.VelocityZ * 3.6f)), 0, 999);
    
    float rpm_range = data_out.EngineMaxRpm - data_out.EngineIdleRpm;
    float rpm_pct = 0.0f;
    if (rpm_range > 0.0f) {
        rpm_pct = std::clamp((data_out.CurrentEngineRpm - data_out.EngineIdleRpm) / rpm_range, 0.0f, 1.0f);
    }

    // Fast stack-allocated string formatting (No std::string heap overhead)
    char gear_buffer[3]{0};
    switch (data_out.Gear) {
        case 0:  SDL_snprintf(gear_buffer, sizeof(gear_buffer), " N"); break;
        case 11: SDL_snprintf(gear_buffer, sizeof(gear_buffer), " R"); break;
    ...
    // ==========================================
    // 3. CACHED TEXTURE RENDERING (The performance lifesaver)
    // ==========================================
    
    // Static state storage to cache textures across frames
    ...
    static SDL_Texture* static_kmh_tex = nullptr; // Never changes, created once
    ...
    if (!cached_speed_tex || SDL_strcmp(last_speed_str, speed_buffer) != 0) {
        ...
        SDL_Surface* surf = TTF_RenderText_Blended(font, speed_buffer, 0, white_muted);
        if (surf) {
    ...
    // --- STATIC "kmh" TEXTURE ---
    if (!static_kmh_tex) {
        SDL_Surface* surf = TTF_RenderText_Blended(font, "KM/H", 0, white_muted);
        if (surf) {
    ...
    // ==========================================
    // 4. DRAW THE REV COUNTER BAR (Pure GPU drawing, already efficient)
    // ==========================================
    float bar_x = WIDTH * 0.05f;
    float bar_y = HEIGHT * 0.75f;
    float bar_max_width = WIDTH * 0.9f;
    float bar_height = WIDTH * 0.1f;
...
//             break;
//         default:
//             gear_str = ' ' + std::to_string(data_out.Gear);
//             break;
//         }
